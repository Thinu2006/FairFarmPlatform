    ...
    <div class="max-w-7xl mx-auto">
            ...
            <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between justify-center">
                    <div class="flex-1 min-w-0">
                        <h1 class="text-xl sm:text-2xl font-bold text-gray-900 font-merriweather">මගේ ඇණවුම්</h1>
                    </div>
                </div>
            </div>
        </header>
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg mt-6 mb-4 p-4 sm:p-6 text-sm">
            <form action="{{ route('farmer.orders.index') }}" method="GET" class="flex flex-col sm:flex-row gap-4">
                <div class="flex-grow">
                    <input type="text" name="search" placeholder="මිලදී ගන්නා නම හෝ වී වර්ගය අනුව සොයන්න"
                ...
                <div class="min-w-[150px]">
                    <select name="status" class="w-full px-4 py-2 border rounded-md focus:outline-none focus:border-green-700">
                        <option value="">සියලු තත්ත්වයන්</option>
                        <option value="pending" {{ request('status') == 'pending' ? 'selected' : '' }}>පොරොත්තුවෙන්</option>
                        <option value="processing" {{ request('status') == 'processing' ? 'selected' : '' }}>පිළිගත්</option>
                        <option value="cancelled" {{ request('status') == 'cancelled' ? 'selected' : '' }}>ප්‍රතික්ෂේප කළ</option>
                    </select>
                </div>
        ...
        @if ($orders->isEmpty())
            <div class="text-center bg-white p-6 rounded-lg shadow-md">
                <p class="text-gray-600 text-lg">ඇණවුම් හමු නොවීය.</p>
            </div>
        @else
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($orders as $order)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {{ number_format($order->quantity, 0) }} කි.ග්‍රෑ
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        රු. {{ number_format($order->total_amount, 2) }}
                                    </td>
                                        ...
                                        @elseif($order->status == 'pending')
                                            ...
                                            @if(isset($order->has_sufficient_quantity) && !$order->has_sufficient_quantity)
                                                <span class="block mt-1 text-xs text-red-600">
                                                    ප්‍රමාණවත් නැත<br>
                                                    අවශ්‍ය: {{ number_format($order->quantity) }}කි.ග්‍රෑ<br>
                                                    ඇත: {{ number_format($order->available_quantity) }}කි.ග්‍රෑ
                                                </span>
                                            @endif
                                        @else
                ...
                @foreach ($orders as $order)
                    <div class="bg-white rounded-lg shadow-md p-4">
                        <div class="flex justify-between items-start mb-3">
                            <div>
                                <span class="text-sm text-gray-500">ඇණවුම #{{ $order->id }}</span>
                        ...
                        <div class="border-t border-b border-gray-100 py-2 mb-3">
                            <div class="grid grid-cols-2 gap-1 text-sm">
                                <div class="text-gray-500">මිලදී ගන්නා:</div>
                                <div>{{ $order->buyer->FullName }}</div>

                                <div class="text-gray-500">ප්‍රමාණය:</div>
                                <div>{{ number_format($order->quantity, 0) }} කි.ග්‍රෑ</div>

                                <div class="text-gray-500">මුළු මුදල:</div>
                                <div>රු. {{ number_format($order->total_amount, 2) }}</div>

                                <div class="text-gray-500">දිනය:</div>
                        ...
                        @if(isset($order->has_sufficient_quantity) && !$order->has_sufficient_quantity && $order->status == 'pending')
                            <div class="mb-3 p-2 bg-red-50 text-red-600 text-xs rounded">
                                <p>ප්‍රමාණවත් නැත</p>
                                <p>අවශ්‍ය: {{ number_format($order->quantity) }}කි.ග්‍රෑ</p>
                                <p>ඇත: {{ number_format($order->available_quantity) }}කි.ග්‍රෑ</p>
                            </div>
                        @endif
