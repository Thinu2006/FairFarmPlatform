        ...
        <div id="deleteModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
            <div class="bg-white rounded-xl shadow-xl p-6 w-full max-w-md mx-4">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-xl font-bold text-gray-900">මකාදැමීම තහවුරු කරන්න</h3>
                    <button onclick="hideDeleteModal()" class="text-gray-500 hover:text-gray-700">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
                <p class="text-gray-700 mb-6" id="deleteModalText">ඔබට මෙම ලැයිස්තුව මකා දැමීමට අවශ්‍ය බව තහවුරු කරන්නද?</p>
        ...
        <div class="bg-white rounded-xl shadow-sm p-4 sm:p-6 mb-6">
            <div class="flex flex-col sm:flex-row justify-between items-center gap-4 text-center sm:text-left">
                <div class="w-full sm:w-auto">
                    <h1 class="text-xl sm:text-2xl font-bold text-gray-900 font-merriweather">මගේ වී ලැයිස්තු</h1>
                </div>
                ...
                <div class="w-full sm:w-auto mt-2 sm:mt-0">
                    ...
                    class="inline-flex items-center justify-center px-3 py-2 sm:px-4 sm:py-3 bg-green-700 hover:bg-green-800 text-white rounded-lg shadow-sm transition-all duration-200 hover:shadow-md transform hover:-translate-y-0.5 text-sm sm:text-base font-medium">
                        ...
                        <span class="whitespace-nowrap">නව ලැයිස්තුවක් එක් කරන්න</span>
                    </a>
                </div>
                @endif
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-4 sm:p-6">
            @if($listings->isEmpty())
                <div class="text-center py-12">
                    ...
                    <h3 class="text-lg sm:text-xl font-bold text-gray-800 mb-2 font-merriweather">වී ලැයිස්තු නොමැත</h3>
                    <p class="text-base text-gray-500 mb-6 max-w-md mx-auto">ඔබ තවමත් විකිණීමට වී වර්ග ලැයිස්තුගත කර නැත.</p>
                    <a href="{{ route('farmer.paddy.listing.form') }}"
                                ...
                                <div class="space-y-3 mb-5">
                                    <div class="flex justify-between items-center">
                                        ...
                                        <span class="text-sm sm:text-base font-bold text-green-700">
                                            රු. {{ number_format($paddy->PriceSelected, 2) }}
                                        </span>
                                    </div>
                                    <div class="flex justify-between items-center">
                                        <span class="text-gray-600 text-sm sm:text-base">
                                        ...
                                        <span class="text-sm sm:text-base font-bold {{ $paddy->Quantity <= 0 ? 'text-red-600' : 'text-green-700' }}">
                                            ...
                                            @if($paddy->Quantity <= 0 && isset($paddy->has_pending_orders) && $paddy->has_pending_orders)
                                                <span class="ml-1 text-xs bg-yellow-100 text-yellow-700 px-1 py-0.5 rounded">
                                                    {{ $paddy->pending_orders_count }} තීරණය වෙමින් පවතින ඇණවුම්
                                                </span>
                                            @endif
                                        </span>
                                    </div>
                                    @if($paddy->Quantity <= 0 && isset($paddy->has_pending_orders) && $paddy->has_pending_orders)
                                        <div class="mt-2 bg-yellow-50 border-l-4 border-yellow-400 p-2 text-xs text-yellow-800">
                                            <p>ඔබට තීරණය වෙමින් පවතින ඇණවුම් ඇති නමුත් තිබෙන ප්‍රමාණය ශුන්‍යය. කරුණාකර මෙම ඇණවුම් සපුරාලීම සඳහා ඔබේ ප්‍රමාණය යාවත්කාලීන කරන්න.</p>
                                        </div>
                                    @endif
                                </div>
...
<script>
    function showDeleteModal(id, paddyName) {
        ...
        text.textContent = `ඔබට "${paddyName}" ලැයිස්තුව මකා දැමීමට අවශ්‍ය බව තහවුරු කරන්නද? මෙම ක්‍රියාව අහෝසි කළ නොහැක.`;
